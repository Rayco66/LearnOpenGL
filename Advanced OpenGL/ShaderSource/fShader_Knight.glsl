...
struct Material
{
	...
	sampler2D AmbientColorSampler1;//环境光反射贴图采样器标号
...
vec3 GetLight_Reflection(Material material,samplerCube SkyBoxTexture,vec3 Normal,vec3 FragPos,vec3 ViewPos);
...
{    
	...
	ResultColor += GetLight_Reflection(material,SkyBoxTexture,Normal,FragPos,ViewPos);
...
vec3 GetLight_Reflection(Material material,samplerCube SkyBoxTexture,vec3 Normal,vec3 FragPos,vec3 ViewPos)
{
	vec3 TextureColor_Reflection = texture(material.AmbientColorSampler1,TexCoord).rgb;//将环境光纹理当作反射纹理使用

	vec3 ViewDir = normalize(FragPos - ViewPos);
	vec3 ViewDir_Reflect = reflect(ViewDir,normalize(Normal));

	vec3 Reflection = vec3(texture(SkyBoxTexture,ViewDir_Reflect)) * TextureColor_Reflection;
	return Reflection;
}