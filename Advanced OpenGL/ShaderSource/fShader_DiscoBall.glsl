...
//光照反射贴图
	/*漫反射*/
	vec3 TextureColor_Diff;
	//判断是否绑定了纹理
	if (texture(material.DiffuseColorSampler1, TexCoord).a == 0.0) TextureColor_Diff = vec3(1.0f); // 默认的漫反射颜色
	else TextureColor_Diff = texture(material.DiffuseColorSampler1, TexCoord).rgb;

	/*镜面反射*/
	vec3 TextureColor_Spec;
	if (texture(material.SpecularColorSampler1, TexCoord).a == 0.0) TextureColor_Spec = vec3(1.0f); // 默认的漫反射颜色
	else TextureColor_Spec = texture(material.SpecularColorSampler1, TexCoord).rgb;
