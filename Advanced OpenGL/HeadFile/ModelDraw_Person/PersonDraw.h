...
//一般属性
extern float PassedTime;//非暂停时经过的时间
...
public:
    ...
    void NormalDraw()
    {
        ...
        ModelShader.setMat4("view", ViewMatrix);
        ModelShader.setMat4("model", ModelMatrix);
        ModelShader.setMat3("NormalMatrix", glm::mat3(glm::transpose(glm::inverse(ViewMatrix * ModelMatrix))));//由于在几何着色器中我们使用的是观察空间中坐标进行计算，所以为了消除观察和模型矩阵的缩放和旋转的影响，要将法向量也变换到观察矩阵中
        ModelShader.SetFloat("NormalLength",0.05f);//法线长度
        //绘制
