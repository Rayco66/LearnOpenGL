...
int main()
{
    ...
    for (unsigned int i = 0; i < ObjectModel_Rock.meshes.size(); i++)
    {
        ...
        glVertexAttribDivisor(3, 1);//每绘制一个实例就更新一次顶点属性(position = 3)
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //小行星模型绘制
        RockShader.Use();
        glm::mat4 model_revolution = glm::mat4(1.0f);//公转旋转模型矩阵
        model_revolution = glm::rotate(model_revolution, glm::radians(5.0f) * PassedTime, glm::vec3(0.0f, 1.0f, 0.0f));
        glm::mat4 model_rotation = glm::mat4(1.0f);//自转旋转模型矩阵
        model_rotation = glm::rotate(model_rotation,glm::radians(45.0f) * PassedTime, glm::vec3(0.0f, 1.0f, 0.0f));//由于此处的rotate是根据单位矩阵model_revolution直接进行变换的，故这里的旋转轴既是局部坐标系y轴，也是世界坐标系y轴

        //计算相关
        RockShader.setMat4("model_rotation", model_rotation);
        RockShader.setMat4("model_revolution", model_revolution);
...
//5.片段着色器:每个生成的片段会传递给片段着色器(不区分具体图元，直接处理所有通过光栅化的片段)，进行进一步处理，如计算颜色和光照等。/**片段着色器中有这个图形的所有像素点
